class Stage
  ...
  def update
    ...
    if enemies_count(:existing) == 0 && enemies_count(:created) >= ENEMIES_COUNT[@level]
      if @level < MAX_LEVEL
        level_up
      else
  ...
  def draw
    @font.draw("Enemies: #{enemies_count :left}/#{ENEMIES_COUNT[@level]}", 890, 495, ZOrder::Text)
  end
  ...
  def level_up
    ...
    @level += 1
    @total_enemies_count = 0
    @window.enemies_destroyed = 0
    if @window.ship
      @window.ship.create_shield false
      ...
      @window.ship.shield_left = PlayerShip::SHIELD_COUNT[@window.level]
    end
  end
    ...
    count.times do
      ...
      while true
        ...
        if enemy
          ...
          enemy = EnemyShip.new(@window, EnemyShip::NAMES.sample, position[:x], position[:y])
        end
        flag = true
